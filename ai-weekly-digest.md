...
- **$ARGUMENTS** (optional): `last week`, `this week` (default), or a date like `2026-03-20`
...
Do a single WebSearch for `"AI news this week {date_range}"` to catch anything the dedicated sources missed. **Do NOT scrape individual results** — just extract titles, URLs, and summaries directly from the search results page. This subagent should be fast (1-2 WebSearch calls max, zero WebFetch calls).
...
**4. Must Read & Must Watch (5 total)** — Pick the 5 most impactful/interesting items across ALL sections. Mix of articles and videos. Prioritize:
